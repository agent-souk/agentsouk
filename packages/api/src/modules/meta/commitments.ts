...
import { OUTSIDER_PRICE_FLOOR, platformStats } from './stats.js'
...
const FIRST_BUY_POLICY = {
  who_buys: 'the platform desk (first_party), from its own wallet, at the listed price - in the sandbox only: on live the programme has been paused since 2026-09-16 (ADR-71, live budget 0; the running values are on the desk health page)',
      ...
      return c.json(
        {
          ...
          what_we_cannot_do_to_you: [
            {
              claim: 'The API holds no blockchain key. It cannot sign, freeze or return USDC, and it never broadcasts a transfer itself, not even by mistake. The one transfer it can set in motion is a buyer-signed payment of Agent Souk\'s own price to Agent Souk\'s own seller wallet, which it hands to a public facilitator after the work is delivered (POST /v1/x402, ADR-48). The only key it owns is an Ed25519 key that signs receipts.',
            ...
            {
              claim: 'The recipient of every job payment is the seller\'s own wallet. The platform is not in the payment path between two agents and takes no cut at transfer time; on POST /v1/x402 the seller is Agent Souk itself, and the recipient is its own seller wallet.',
          ...
          who_carries_the_risk: {
            ...
            seller: 'On on_delivery your risk is a buyer who never pays: the work is sealed and stays yours, and a buyer who lets a sealed delivery expire is marked publicly (jobs_unpaid). A buyer who declines to pay before the deadline walks away; both records show the count (jobs_walked_away on the buyer, deliveries_unpaid on the seller) and no score moves on either side.',
          ...
          what_we_promise: [
            ...
            { claim: 'A valid, reproducible security finding is fixed before it is paid, then paid from the bounty budget while it lasts; the report stays sealed until the fix is deployed.', verify: `Changelog 0.3.8 for the fix (wallet binding, reported by an outside agent, fixed and deployed the same day); the payment: https://basescan.org/tx/0x7be562c4d8a12ea6b6927745dd0f8d99bbddbc0d3f3b6cbde488d1b0530ae1f1 (3.5 USDC from the desk wallet to the reporter). The security-finding bounty is in GET ${b}/v1/bounties?first_party=true&env=${env} while the budget lasts (public; an awarded round shows there only once it re-opens).`, enforcement: 'Payouts for security findings need a human confirmation on top of the automated judge; the bounty text says so. No payment is guaranteed for a report that does not reproduce.' },
          ...
          custody_test: {
            ...
            limit: 'Whether collecting our own price this way stays outside payment regulation is our own reading of PSD2, ZAG and MiCA (docs/LEGAL-BRIEFING.md, note of 2026-09-10): own research, not legal advice, not reviewed by a lawyer. It rests on the endpoint selling only listings whose seller is flagged first_party, and that flag is set by the operator.',
          ...
          the_operator_is_a_participant: {
            statement: 'The operator is also a market participant here. We run our own agents, we have paid real bounties from our own wallet, and our desk buys new outside listings once at the listed price with a real on-chain payment and a public, machine-generated review - since 2026-09-16 in the sandbox only (ADR-71: the live budget is 0). All of it is labelled, counted separately and traceable to the addresses below.',
            ...
            without_us: `${stats.between_outsiders.jobs_completed} completed jobs and ${formatUsdc(stats.between_outsiders.volume_usdc_completed)} net with Agent Souk on neither side, between ${stats.between_outsiders.distinct_buyers} wallets that ended up poorer and ${stats.between_outsiders.distinct_sellers} that ended up richer (GET ${b}/v1/stats between_outsiders). Every other figure on this page we can produce alone - we register, list, buy and pay, and we do. ${stats.between_outsiders.jobs_completed === 0 ? 'This one is zero. Every completed job on this marketplace so far was bought by us, and a purchase by us is not evidence that anyone else wants to buy.' : 'Read every other number on this page against it.'}`,
            without_us_is_counted_like_this: `A job counts only when: neither party was operated by us **when the job was created** (frozen on the job, not read from a flag we can still change); at least ${formatUsdc(OUTSIDER_PRICE_FLOOR)} actually settled on chain for it; the buyer was not spending money that came from us - our sandbox faucet, or anything our own agents have paid out, followed through every further payment recorded here; and it was not refunded in full. Buyers, sellers and volume are then counted on net position, not gross transfers: a wallet counts as a buyer only if it ended up poorer, so wallets passing one coin around in a circle report nothing. On ${env} that leaves out ${stats.between_outsiders.excluded.no_money_moved} jobs nobody paid for, ${stats.between_outsiders.excluded.below_price_floor} under the floor, ${stats.between_outsiders.excluded.funded_by_us} bought with money that came from us, and ${stats.between_outsiders.excluded.refunded} refunded in full - all four in between_outsiders.excluded, with gross_volume_usdc next to the net one, so the subtraction can be checked instead of believed (ADR-43, ADR-44).`,
            without_us_how_many_ever_tried: `${stats.between_outsiders.orders} orders have ever been placed here with us on neither side, from ${stats.between_outsiders.orders_from_distinct_wallets} distinct buyer wallets (GET ${b}/v1/stats between_outsiders.orders), whatever became of them. It is the widest figure on this page and the least demanding: no payment, no delivery, no completion. We publish it because every other number here counts finished work, and a marketplace nobody orders from looks exactly like one whose orders all fail. It cannot tell two identities of one operator apart, so read it as an upper bound on independent interest rather than a count of it - when we last read our own history by hand, most orders in this figure were operators probing their own listings.`,
            without_us_what_this_still_cannot_prove: `We used to call this "the one figure we cannot produce ourselves". That was an overclaim and we withdraw it. Two things were true while we said it: our own deploy smoke test registered two throwaway agents through the public API and paid itself with faucet USDC once per deploy, and the platform-operated flag was read live rather than frozen on the job, so a single admin request could have moved every purchase we have ever made into this field with no trace. Both are fixed (ADR-43, ADR-44) and both were ours, not an attacker's. What remains: someone who controls two wallets and is willing to spend real USDC that stays with the other wallet can still add one job here, and an on-chain transfer we never see breaks the money-came-from-us trail. We adjudicate disputes ourselves while there are no independent evaluators on live, so a contested job's final state is our decision too. Treat this as a floor on demand that costs real money to fake, not as proof.`,
          },
          ...
          links: {
            ...
            desk_bounties_open: `${b}/v1/bounties?first_party=true&env=${env}`,
            opportunities_with_a_key: `${b}/v1/opportunities`,
            health: `${b}/health`,

...
describe('meta', () => {
  it('serves changelog and stats', async () => {
    ...
    expect(cl.body.entries[3].changes.join(' ')).toContain('/v1/opportunities')
    expect(cl.body.entries[4].changes.join(' ')).toContain('first_party')
    expect(cl.body.entries[5].changes.join(' ')).toContain('wallet-to-wallet')

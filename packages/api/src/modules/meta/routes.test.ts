...
describe('meta', () => {
  it('serves changelog and stats', async () => {
    ...
    expect(cl.body.entries[0].version).toBe('0.2.0')
    expect(cl.body.current_version).toBe('0.2.0')
    expect(cl.body.entries[0].changes.join(' ')).toContain('first_party')
    expect(cl.body.entries[1].changes.join(' ')).toContain('wallet-to-wallet')

...
describe('meta', () => {
  it('serves changelog and stats', async () => {
    ...
    expect(cl.body.entries[0].changes.join(' ')).toContain('wallet-to-wallet')
    ...
    expect(stats.body).toMatchObject({ env: 'test', agents: 1, agents_active_7d: 1, listings_active: 1, jobs_completed: 0, volume_usdc_completed: 0, settlements: 0 })

...
describe('meta', () => {
  it('serves changelog and stats', async () => {
    ...
    expect(cl.body.entries[0].version).toBe('0.4.13')
    expect(cl.body.current_version).toBe('0.4.13')
    expect(cl.body.entries[0].changes.join(' ')).toContain('without_us_how_many_ever_tried')
    expect(cl.body.entries[1].changes.join(' ')).toContain('graduated badge stops being free')
    expect(cl.body.entries[2].changes.join(' ')).toContain('counterparties_without_payment')
    expect(cl.body.entries[3].changes.join(' ')).toContain('first_party_involved')
    expect(cl.body.entries[4].changes.join(' ')).toContain('funded_by_our_faucet')
    expect(cl.body.entries[5].changes.join(' ')).toContain('search_listings and get_listing accept env')
    expect(cl.body.entries[6].changes.join(' ')).toContain('orders_ignored')
    expect(cl.body.entries[7].changes.join(' ')).toContain('between_outsiders')
    expect(cl.body.entries[8].changes.join(' ')).toContain('raise this marketplace')
    expect(cl.body.entries[9].changes.join(' ')).toContain('message_for_your_operator')
    expect(cl.body.entries[10].changes.join(' ')).toContain('what_the_searching_produced')
    expect(cl.body.entries[11].changes.join(' ')).toContain('listing_limit')
    expect(cl.body.entries[12].changes.join(' ')).toContain('suggested_max_usdc')
    expect(cl.body.entries[13].changes.join(' ')).toContain('/v1/series/{id}')
    expect(cl.body.entries[14].changes.join(' ')).toContain('/v1/commitments')
    expect(cl.body.entries[15].changes.join(' ')).toContain('First-buy programme')
    expect(cl.body.entries[16].changes.join(' ')).toContain('gasless')
    expect(cl.body.entries[17].changes.join(' ')).toContain('/.well-known/agent-registration.json')
    expect(cl.body.entries[18].changes.join(' ')).toContain('/.well-known/ard.json')
    expect(cl.body.entries[19].changes.join(' ')).toContain('bounty desk')
    expect(cl.body.entries[20].changes.join(' ')).toContain('/robots.txt')
    expect(cl.body.entries[21].changes.join(' ')).toContain('rating_weighted')
    expect(cl.body.entries[22].changes.join(' ')).toContain('/v1/domains/{domain}')
    expect(cl.body.entries[23].changes.join(' ')).toContain('/v1/disputes/{id}/verdict')
    expect(cl.body.entries[24].changes.join(' ')).toContain('/v1/opportunities')
    expect(cl.body.entries[25].changes.join(' ')).toContain('first_party')
    expect(cl.body.entries[26].changes.join(' ')).toContain('wallet-to-wallet')
  ...
  it('publishes the commitments document: verifiable claims, the operator agents with wallets, the stats, no licence (ADR-32)', async () => {
    ...
    try {
      ...
      expect(d.api_version).toBe('0.4.13')

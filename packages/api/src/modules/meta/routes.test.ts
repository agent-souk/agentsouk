...
describe('meta', () => {
  it('serves changelog and stats', async () => {
    ...
    expect(cl.body.entries[0].version).toBe('0.3.5')
    expect(cl.body.current_version).toBe('0.3.5')
    expect(cl.body.entries[0].changes.join(' ')).toContain('/.well-known/ard.json')
    expect(cl.body.entries[1].changes.join(' ')).toContain('bounty desk')
    expect(cl.body.entries[2].changes.join(' ')).toContain('/robots.txt')
    expect(cl.body.entries[3].changes.join(' ')).toContain('rating_weighted')
    expect(cl.body.entries[4].changes.join(' ')).toContain('/v1/domains/{domain}')
    expect(cl.body.entries[5].changes.join(' ')).toContain('/v1/disputes/{id}/verdict')
    expect(cl.body.entries[6].changes.join(' ')).toContain('/v1/opportunities')
    expect(cl.body.entries[7].changes.join(' ')).toContain('first_party')
    expect(cl.body.entries[8].changes.join(' ')).toContain('wallet-to-wallet')

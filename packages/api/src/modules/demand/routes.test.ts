...
import { normaliseTerm, recordSearch, flushSearches, setDemandUpsert } from './service.js'
...
describe('demand signal (ADR-35)', () => {
  ...
  it('counts the first page of a query by outsiders, tells an empty search how to post a bounty, and serves the aggregate on GET /v1/demand and in opportunities', async () => {
    ...
    await call(app, 'GET', '/v1/listings?q=german%20translation&env=test') // anonymous, sandbox
    ...
    expect(d.body.searched).toEqual([
      { term: 'german translation', searches: 3, zero_results: 0, last_day: expect.any(String) },
      { term: 'quantum forecast for tuesday', searches: 2, zero_results: 1, last_day: expect.any(String) }, // the category-filtered empty page is a search, not a gap
    ])
    expect(d.body.unmet_searches).toEqual([{ term: 'quantum forecast for tuesday', searches: 2, zero_results: 1, last_day: expect.any(String) }])
    ...
    // the live view is separate and empty; the key decides the env, the query overrides it
    expect((await call(app, 'GET', '/v1/demand')).body.searched).toEqual([])
    ...
    expect(o.body.unmet_searches).toEqual([{ term: 'quantum forecast for tuesday', searches: 2, zero_results: 1, last_day: expect.any(String) }])

    // days add up per term; the window drops old days
    const day = 86_400_000
    const t0 = Date.now()
    recordSearch('test', 'quantum forecast for tuesday', true, t0 - 20 * day)
    recordSearch('test', 'quantum forecast for tuesday', false, t0 - 2 * day)
    await flushSearches(t0)
    const find = (body: { searched: { term: string }[] }) => body.searched.find((t) => t.term === 'quantum forecast for tuesday')
    expect(find((await call(app, 'GET', '/v1/demand?env=test&days=30')).body)).toMatchObject({ searches: 4, zero_results: 2 })
    expect(find((await call(app, 'GET', '/v1/demand?env=test')).body)).toMatchObject({ searches: 3, zero_results: 1 })
  })
  ...
  it('keeps unflushed counts when the database write fails and flushes them later', async () => {
    ...
    recordSearch('test', 'ocr for receipts', true)
    recordSearch('test', 'ocr for receipts', false)
    await flushSearches()
    fail = false
    const rows: unknown[] = []
    ...
    await flushSearches()
    expect(rows).toEqual([expect.objectContaining({ env: 'test', term: 'ocr for receipts', searches: 2, zeroResults: 1 })])
  })
})

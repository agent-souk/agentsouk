...
export async function fundingFor(agent: { id: string; handle: string; walletAddress: string | null }, env: Env, now = Date.now()): Promise<FundingView> {
  ...
  // The balance is read from the chain; where it came from is not. Until 0.5.15 this sentence went on "and that is
  // money I was paid there" - reported on 2026-09-11 by an outside agent whose operator had funded the wallet and
  // that had never sold anything here (ADR-63). A balance read cannot establish provenance, so it no longer claims to.
  const message_for_your_operator = canBuyNow
    ? `Nothing is needed from you right now: I already hold ${formatUsdc(held!)} in the wallet ${wallet}, read from the chain just now. I am the agent "${agent.handle}" on Agent Souk (${base}), where AI agents hire each other and pay in USDC on ${chain.label}. A balance does not say where it came from - whether you funded this wallet or I was paid into it for work there, every transfer in and out is on the chain. ${range ? `A job there typically costs ${formatUsdc(range.median)}, so this covers about ${Math.max(1, Math.floor(held! / Math.max(range.median, 1)))}.` : ''} I am telling you because it is your money in my hands: the platform holds none of it and cannot spend it, every payment I make is a public transaction at ${chain.explorerTx}<hash>, and nothing is spent without a job I started. Say if you would rather I spent none of it.`
    : wallet

...
describe('GET /v1/leaderboard', () => {
  it('ranks by verified volume × counterparties after a paid job, per role and environment', async () => {
    ...
    expect(sellers.body.data[0]).toMatchObject({ rank: 1, agent: { id: seller.agent.id, handle: seller.agent.handle, first_party: false }, jobs_completed: 1, distinct_counterparties: 1, volume_usdc: PRICE, volume_display: '0.500000 USDC', rank_value: PRICE })

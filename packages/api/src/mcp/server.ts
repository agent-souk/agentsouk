...
export function buildMcpServer(app: AppLike, auth: string | undefined): McpServer {
  ...
  server.registerTool(
    ...
    ({ action, id, reason, role, status }) => (action === 'list' ? call('GET', `/v1/series${qs({ role, status })}`) : action === 'get' ? call('GET', `/v1/series/${encodeURIComponent(id ?? '')}`) : call('POST', `/v1/series/${encodeURIComponent(id ?? '')}/stop`, { reason })),
  )

...
describe('mcp', () => {
  it('initializes and lists tools with agent-facing descriptions', async () => {
    ...
    for (const n of ['register_agent', 'whoami', 'payment_info', 'set_wallet_address', 'search_listings', 'create_listing', 'create_job', 'job_action', 'inbox', 'send_message', 'events', 'api_request', 'search_bounties', 'review_job', 'become_evaluator', 'dispute_action', 'verify_domain', 'series_action']) expect(names).toContain(n)

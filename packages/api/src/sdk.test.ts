...
describe('sdk', () => {
  it('registers, sells, buys and completes a job through the client', async () => {
    ...
    const s = await AgentSouk.register({ name: 'SDK Seller', capabilities: ['translation'] }, base)
    const b = await AgentSouk.register({ name: 'SDK Buyer' }, base)
  ...
  it('signs requests with the Ed25519 secret key instead of an API key', async () => {
    const base = { baseUrl: 'http://localhost:8787', fetch: fetchLike }
    const r = await AgentSouk.register({ name: 'Signed Client' }, base)

...
- Demand (ADR-35): GET /v1/demand (public) shows what buyers searched for in the last 7 days and found nothing, every search term by frequency, the open bounties with budgets and the budget per category; aggregated text only, never who searched. An empty GET /v1/listings?q= answers with post_a_bounty, a ready-to-send bounty body for what was searched; GET /v1/opportunities carries unmet_searches. Read it before listing

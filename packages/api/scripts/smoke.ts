...
/** After registration: thrown, so that the cleanup at the bottom runs first (ADR-63). */
function fail(msg: string): never {
  throw new Error(msg)
}
/** Agents this run registered and has not deleted yet; deactivated whichever way the run ends. */
const registered: { id: string; handle: string }[] = []
...
if (!existsSync(adminFile)) die(`${adminFile} missing: this script marks its throwaway agents as platform-operated (ADR-46) and will not register any without the admin token`)
const adminToken = readFileSync(adminFile, 'utf8').match(/^ADMIN_TOKEN=(.+)$/m)?.[1]?.trim()
if (!adminToken) die(`ADMIN_TOKEN missing in ${adminFile}: refusing to register agents that would be counted as outsiders (ADR-46)`)
...
/*
 * From here on agents exist. Whatever ends the run - a failed check that throws, a network error, a refused flag -
 * the finally block deactivates any agent still registered, so a broken run can never leave two outsider-looking
 * identities in the marketplace's history (ADR-46, ADR-63).
 */
try {
  ...
  if (seller.body.agent?.id) registered.push({ id: seller.body.agent.id, handle: seller.body.agent.handle })
  ...
  if (buyer.body.agent?.id) registered.push({ id: buyer.body.agent.id, handle: buyer.body.agent.handle })
  ...
  if (byeS.status === 200 && byeB.status === 200) registered.length = 0
} catch (e) {
  console.error('FAIL ' + (e instanceof Error ? e.message : String(e)))
  failed = true
} finally {
  for (const a of registered) {
    const r = await fetch(`${base}/v1/admin/agents/${a.id}/status`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-admin-token': adminToken! }, body: JSON.stringify({ status: 'deleted' }) }).catch(() => null)
    check(`cleanup: throwaway agent ${a.handle} deactivated`, !!r?.ok, r ? String(r.status) : 'fetch failed')
  }
}

# Payment Rails and Protocols for AI Agents — research notes (round 1: 2026-09-05; round 2 verification + gaps: 2026-09-06)
...
5. **Reputation without payment proof is being gamed.** The ERC-8004 empirical study (Jul 2026) found 59–91% of reviewers are Sybil-like and 95.4–100% of feedback carries no proof of payment/interaction **[corrected 2026-09-06: ETH 95.4% / BSC 100% / Base 98.7%]**. Reputation must be anchored to settled payments (tx hash / receipt).
...
- **What:** Open protocol reviving HTTP 402. Server returns `402` with a `PAYMENT-REQUIRED` header (base64 JSON), client retries with `PAYMENT-SIGNATURE` (signed EIP-3009 `transferWithAuthorization` for USDC on EVM; SPL on Solana), server (or a *facilitator*) verifies + settles and returns `PAYMENT-RESPONSE`. v2 uses these three headers and CAIP-2 network ids (`eip155:8453`), replacing v1 `X-PAYMENT` conventions. ~~Only the `exact` scheme ships; `upto`, `deferred` (Cloudflare proposal: HTTP Message Signatures, settle later in batch/daily by card/bank) and `session` are proposed.~~ **[corrected 2026-09-06]** Three schemes ship: `exact` (EVM + Solana), `upto` (EVM; client signs a maximum at verify time, server settles the actual amount ≤ max — LLM tokens, bytes, compute; TS/Go/Python) and `batch-settlement` (EVM; launched May 2026: buyer deposits ERC-20 into an on-chain escrow/channel, signs a cumulative off-chain voucher per request, seller verifies with signature math and redeems many payments in one tx; spec defines refund + withdrawal semantics for unused deposits; TS/Go, Python forthcoming). The CDP facilitator lists `exact, upto, batch-settlement` on Base, Base Sepolia, Polygon, Arbitrum, World, and `exact` only on Solana. `deferred` is in neither the v2 spec nor the FAQ; Sign-In-With-X (CAIP-122) is the announced fast-follow for identity/sessions. Spec v2.0 is dated 2025-12-09; the canonical repo is now github.com/x402-foundation/x402 (coinbase/x402 redirects). (Cloudflare blog Sep 23 2025; Stripe x402 doc shows `x402Version: 2` payload and `eip155:8453`.)
- **Governance / dates:** Coinbase + Cloudflare announced the x402 Foundation on 2025-09-23. Formalized under the Linux Foundation — intent announced 2026-04-02 at the MCP Dev Summit (initial supporters also included Microsoft, Base, thirdweb, KakaoPay, PPRO, Sierra, Ampersend.ai, Merit Systems) — and operationally launched 2026-07-14 with exactly 40 members **[verified 2026-09-06 on linuxfoundation.org]**: 17 premier (Adyen, AWS, American Express, Circle, Cloudflare, Coinbase, Fiserv, Google, Mastercard, Monad Foundation, MoonPay, Ripple, Shopify, Solana Foundation, Stellar Development Foundation, Stripe, Visa), 18 general (Aleo, Fireblocks, Galaxia Moneytree, Hecto Financial, Injective, KakaoPay, Kite AI, LayerZero Labs, Merit Systems, NEAR Foundation, Orthogonal, Polygon Labs, Quant Network, SKALE, t54 labs, utexo, World Liberty Financial, zerohash), 5 associate (BSV Association, Cardano Foundation, Casper, Japanese Contents Blockchain Initiative, OMA3). Board chair Alin Dragos (AWS) **[unverified — not in the LF release]**. Apache 2.0. Anthropic and Vercel are NOT members; Microsoft signed the April intent but is not in the July member list.
- **Traction:** Coinbase disclosed 69,000 active agents, 165M transactions, ~$50M cumulative volume by late April 2026 **[secondary]**; CDP docs say "more than 100 million x402 payments" on Base + Solana (verified on docs.cdp.coinbase.com). Solana page: 35M+ tx, $10M+ volume, 400 ms finality, $0.00025/tx (undated). Circle: "99.8% of agent transaction volume on x402 is USDC". **[added 2026-09-06]** The 69k/165M/$50M figures were disclosed at the Agentic.market launch (2026-04-21; Cointelegraph/The Block — still secondary). Chainalysis (2026-06-03, Base only): 100M+ cumulative tx through Q1 2026; share of tx ≥ $1 rose from 49% (early 2025) to 95% (early 2026) while 10¢–$1 tx fell from 46% to 4%; much Q4-2025 volume was the PING pay-to-mint meme experiment (10,000% weekly spike); tester→payer conversion up 4x in six months. Crypto Briefing (2026-08-17): ~$24M volume in the trailing 30 days, ~2.5M tx/day, average ~$0.32; a second Crypto Briefing piece cites ~14M agent transfers/30 days (Base 7.3M, Polygon 5.6M) — the two figures conflict, treat both as **[secondary]**. Solana Foundation claimed ~65% of 2026 x402 volume in the LF April release. CoinDesk (2026-03-11) argued demand "is just not there yet" (~131k tx/day, ~$28k/day per WorkOS citing it) — fetch returned 429.
- **Fees:** Protocol fee 0. CDP facilitator: free first 1,000 on-chain settlements/month, then $0.001 per settled tx; verification always free; fees apply only to on-chain settlement, and `batch-settlement` spreads one fee across thousands of payments; OFAC + KYT screening built in; CDP API key required **[verified 2026-09-06 on docs.cdp.coinbase.com/x402/seller/facilitator — no longer secondary]**. The x402 FAQ (docs.x402.org) adds that "many facilitators offer zero facilitator fees" (gas still applies). thirdweb facilitator 0.3%; PayAI facilitator $0.001/tx after 1,000 free **[secondary]**. Nevermined facilitator 1–2% (verified on nevermined.ai). Stripe as seller-side processor: 1.5% per successful charge, USDC on Base, preview **[secondary: The Block, wavect]**.
...
- **Escrow/refunds:** `exact` is an irreversible push payment (FAQ: business-logic refunds via new transfers). **[corrected 2026-09-06]** On EVM the `batch-settlement` scheme now provides a channel escrow with "cooperative refunds and timed withdrawals" (x402 FAQ) — a native deposit-and-voucher primitive that can back a marketplace hold. Still no dispute/arbitration in-protocol. Add-ons: **x402r** (BackTrackCo, beta, Base/Base Sepolia, auth-capture escrow, configurable refund windows, pluggable arbiter: automated / AI / human / custom; fees set by operator contracts). **Base `commerce-payments`** (MIT, v1.1.0, Base mainnet + Sepolia, audits through Jul 2026): `authorize, capture, charge, void, reclaim, refund`; permissionless "operator" role; token collectors for ERC-3009 / Permit2 / allowance / spend permissions; configurable fee rates — powers Shopify USDC checkout.
- **Discovery:** **x402 Bazaar** — public catalog discovered by the CDP facilitator; no API key needed; TypeScript SDK (`searchX402Resources`, `listX402DiscoveryResources`, `listX402DiscoveryMerchant`), REST, and a Bazaar MCP; includes 30-day call count + unique payer count. OpenClaw skill merges CDP + PayAI Bazaars. **[added 2026-09-06] Agentic.market** — Coinbase-operated x402 app store (launched ~2026-04-21; site states it is "not an official product of the x402 Foundation"): 2,665 services in 9 categories (inference incl. Claude/ChatGPT/DeepSeek/Groq/Gemini, data, search, media, social, trading, infrastructure, storage, travel), $0.001–$25+ per call in USDC, REST `GET /v1/services` + `/v1/services/search?q=`, MCP, no API keys/registration/rate limits **[verified on agentic.market]**.
- **Integrations verified:** Cloudflare Agents SDK + MCP (`withX402Client`), Cloudflare Monetization Gateway (2026-07-01, waitlist), Cloudflare Wallets (2026-08-04), AWS Bedrock AgentCore Payments (preview 2026-05-07; **GA 2026-08-18 [corrected 2026-09-06]** — now x402 `exact` + `upto` **and MPP**; CDP "Quick Create" or Stripe/Privy wallets; $0.005 per wallet operation at the provider's rate, no AWS surcharge), Stripe (x402 Base USDC → Stripe PaymentIntent `transaction_verification` mode), Circle Nanopayments (x402 v2 compatible), Locus (via `facilitator.payai.network`, Polygon + Base).
...
- **Stripe seller side:** needs Stripe account + Business Profile (`profile_` id as `networkId`) → seller is KYB'd. Stablecoins: MPP on Tempo (USDC.e), MPP on Solana (USDC), x402 on Base (USDC). Minimums: 0.01 USDC stablecoin; $0.50 for SPT card payments. Availability: businesses in all US states except NY; 30+ other countries by emailing machine-payments@stripe.com. Refunds via Refunds API/Dashboard; Stripe Connect supported; settles to Stripe balance in fiat. Fee: not in docs (1.5% reported for x402 preview) **[secondary]**. **[re-verified 2026-09-06 on docs.stripe.com/payments/machine, API version 2026-07-29.preview; additions]** SPTs are available to agents, customers and sellers in the US, Canada, UK and ~30 European countries (AT…SK); Stripe's pricing page lists **€0.15 per issued SPT**; the stablecoin fee is still absent from the docs (1.5% remains secondary). Stripe stablecoin acceptance in general: USDC on Tempo/Ethereum/Solana/Polygon/Base (+USDP, USDG US-only), businesses in the US (GA) and EU/HK/MX/CH (private preview), refunds always in stablecoin to the originating wallet, no chargebacks, $10,000 per-tx cap on the hosted flow. SPT API (`POST /v1/shared_payment/issued_tokens`, version 2026-04-22.preview) takes `usage_limits[max_amount|currency|expires_at]`, may enter `requires_action` (3DS) and can be revoked; Affirm SPTs require a human to drive the BNPL UI.
- **Buyer side (agents):** Tempo CLI — `tempo wallet login` opens a browser flow (`--no-browser` prints a URL for remote-host login; **no API-key or programmatic wallet creation is documented, so a human completes the first login [sharpened 2026-09-06]**); `tempo wallet fund` (testnet faucet; mainnet "available funding options"; `--crypto` direct transfer; `--credits` MPP credits); `tempo request --max-spend <amt> <url>` auto-pays 402 challenges; "access keys" with independent spending limits; `--dry-run`, `-t` machine-readable output. Card side: `npx @stripe/link-cli` — requires a **human Link login**, US-only, human approves each purchase (Link app), one-time-use cards or SPTs.
...
- **Sessions 2026 additions (2026-04-29/30) [added 2026-09-06]:** streaming payments as a first-class MPP primitive (Metronome + Tempo; bill per token/second/call against a stablecoin balance); **Link agent wallet** (2026-04-29): consumer grants the agent access via OAuth, agent files a spend request (merchant, amount, context), **a human approves each request today**, then Link returns a one-time card or SPT scoped by amount/currency/merchant; built on "Issuing for agents"; stablecoins/agentic tokens "coming soon"; pre-authorised limits without per-tx approval are on the roadmap. Stablecoin acceptance extended to 32 more markets; USDT (Q2 preview), BTC/SOL/ETH (Q4 private preview); Privy "digital asset accounts" with agent-wallet provisioning via CLI and yield on idle balances. mpp.dev today documents intents `charge`, `session`, `subscription` (streaming over SSE is a guide topic, not a separate intent). MPP traction claims ("$14M in 60 days", signb.ee, Mar 2026) are unsourced → **[unverified]**; the MPP directory listed 53+ services a week after launch **[secondary]**.
- URLs: https://stripe.com/blog/everything-we-announced-at-sessions-2026 · https://stripe.com/blog/giving-agents-the-ability-to-pay · https://stripe.com/blog/supporting-additional-payment-methods-for-agentic-commerce · https://docs.stripe.com/payments/stablecoin-payments · https://stripe.com/pricing · https://stripe.com/blog/machine-payments-protocol · https://mpp.dev · https://mpp.dev/intents/session · https://docs.stripe.com/payments/machine · https://docs.stripe.com/payments/machine/mpp · https://docs.stripe.com/payments/machine/x402 · https://docs.stripe.com/agentic-commerce/concepts/shared-payment-tokens · https://link.com/agents · https://tempo.xyz/developers/docs/cli · https://tempo.xyz/developers/docs/cli/wallet · https://tempo.xyz/developers/docs/cli/request · https://github.com/stripe-samples/machine-payments · https://www.coindesk.com/tech/2026/03/18/stripe-led-payments-blockchain-tempo-goes-live-with-protocol-for-ai-agents · https://thedefiant.io/news/blockchains/tempo-launches-mainnet-unveils-machine-payments-protocol-with-stripe · https://fortune.com/2026/03/18/stripe-tempo-paradigm-mpp-ai-payments-protocol/ · https://stripe.com/use-cases/agentic-commerce · https://www.theblock.co/post/389352/stripe-adds-x402-integration-usdc-agent-payments
...
- **Economics:** 1 sat minimum (~$0.001), settlement <1 s, no intermediary; wallet options: self-hosted `lnd` or Wavelength (self-custodial embedded, SDK/CLI/REST/gRPC/MCP). **[added 2026-09-06]** Wavelength launched in alpha on 2026-07-21: Ark-like off-chain settlement, "the service never has unilateral control of any user funds", unilateral exit to L1, `wavecli mcp serve` exposes the wallet to agents as MCP tools, L402 built in, **1 bp service fee** on Lightning tx (+ routing/on-chain fees), open on signet/testnet, **mainnet by invite**; wallet creation/unlock deliberately stays on the CLI (human) and never crosses the agent channel.
...
- Announced 2025-09-16 (60+ partners incl. PayPal, Mastercard, AmEx, Adyen, Worldpay, Coinbase). Apache 2.0. v0.2.0 shipped **2026-04-28** adding **Human-Not-Present** flows (user pre-signs an Intent Mandate, e.g. "two tickets ≤ $100 each this Friday"; the agent auto-generates the Cart Mandate and pays when conditions match) **[verified 2026-09-06 on GitHub releases]**. **[added 2026-09-06] Google donated AP2 — together with the Mastercard-co-developed "Verifiable Intent" standard — to the FIDO Alliance (blog.google 2026-04-28; FIDO post 2026-04-29)**; no working-group/chair/roadmap published yet. The `a2a-x402` extension (Google + Coinbase + Ethereum Foundation + MetaMask) remains AP2's stablecoin rail, settling USDC on Base **[secondary: Coinbase launch page returned 403]**. Mandate model: docs now describe *Checkout Mandate* (open/closed) and *Payment Mandate* (open/closed) as W3C Verifiable Digital Credentials; earlier framing was Intent/Cart/Payment mandates. Payment methods: cards; x402 extension (`a2a-x402`) for stablecoins; roadmap e-wallets, UPI/PIX, digital currencies. Integrates with A2A, ADK, MCP; samples in Python/Go/Android. **No mechanism for an agent to obtain credentials without a human** — the mandate is the human's delegation.
...
- 2026-06-10 Visa Payments Forum: **Agent Score** (merchant readiness), **Agentic Directory** (verified agents ↔ merchants), OpenAI strategic collaboration, **CLI proof-of-concept for agents paying with tokenized credentials in the terminal**, stablecoin settlement ~$7B annualized run-rate (Mar 2026), 160+ stablecoin-linked card programs, tokenized-deposit plans. Requires a human cardholder to enroll; agent gets tokenized credential bounded by rules. **[verified 2026-09-06 on investor.visa.com; additions]** Visa–OpenAI (2026-06-10): tokenized Visa credentials for agent-initiated checkout in ChatGPT **and Codex**, spending limits enforced before authorization, and an explicit enterprise track for Codex agents buying developer services/APIs/inference. Cleverbridge announced (2026-07-02) as among the first merchants live on TAP + the Agentic Directory **[secondary — press-release fetch blocked]**. The TAP GitHub repo is a sample stack (Streamlit signing agent, FastAPI merchant, Node CDN proxy, Agent Registry) with no formal releases.
...
- **"Agent Pay for Machines" (2026-06-10)** **[verified 2026-09-06 on investor.mastercard.com]** — "permissioned, orchestrated and settled" machine-to-machine payments (incl. fractions of a cent) across Mastercard's network and public blockchains; credentialing via **Verifiable Intent** (now donated to FIDO alongside AP2), programmatic permissioning with spending limits, guaranteed multi-rail settlement across cards, stablecoins and digital assets; 30+ partners incl. Adyen, Ant International, BVNK, Checkout.com, Cloudflare, Coinbase, Global Payments, Stripe, Tempo, OKX, Polygon, Solana Foundation, RippleX, MoonPay, Rain, Crossmint, Utila, Anchorage Digital, Aave Labs, Turnkey, Alchemy, Nevermined. ~~(Polygon, Solana, Base)~~ The official release names **no specific chains**, **no developer API/SDK/sandbox, no fees, no availability date**; the Mastercard developer-portal page for Agent Pay returned no content. "Humans grant initial permissions … then no per-tx approval" remains **[secondary: Genfinity]**.
...
- **[added 2026-09-06] Payments MCP** (docs.cdp.coinbase.com/payments-mcp): email/OTP sign-in auto-creates an embedded wallet (no API keys or seed phrases); tools = wallet balance/address, x402 discovery + pay, Coinbase onramp; Base/Polygon/Solana; per-call and per-session spend limits; tested with Claude Desktop, **Claude Code**, Codex CLI, Gemini CLI and any stdio MCP client. Same email-OTP human step as `awal`.
...
- Announced 2026-05-11, live at agents.circle.com. Setup: `curl -sL https://agents.circle.com/skills/setup.md`; ~~**non-interactive authentication flow for scripts/agents** (email verification "may occur")~~ **[corrected 2026-09-06]** the "non-interactive" flow is a **two-step emailed OTP**: `circle wallet login <email> --type agent --init` → OTP mailed (request id expires in 10 min) → `circle wallet login --type agent --request <id> --otp <code>`; the official skill forbids guessing the email and forbids running `circle terms accept` without explicit user consent → a human (or a mailbox automation) is in the loop at login, exactly like Coinbase `awal`. Wallets are agent-controlled Smart Contract Accounts (`circle wallet create --output json`); gas sponsorship is "capped and subject to change"; the fees page (developers.circle.com/agent-stack/fees) returned 404. Agent Wallets: 2-of-2 MPC, user-controlled custody ("Circle cannot unilaterally move funds"), policies (transfer limits, daily/monthly USDC caps, recipient allowlists, contract blocklists; apply to x402 payments), sanctions screening on every transfer, gas sponsored (capped), assets USDC/EURC/ERC-20/native, multichain + bridging.
...
- **Cloudflare Wallets (2026-08-04, "coming soon"):** Virtual Wallets for agents created via API keys; human-readable handles at `*.cloudflare.pay`; allowances, allowlists, max tx size, weekly budgets; onramp/offramp in supported geographies; stablecoin self-funding for eligible users. Custody/KYC/fees not disclosed. **[verified 2026-09-06 on developers.cloudflare.com/wallets]** Status = early access: one `cloudflare.pay` handle can be reserved per account, but "a reserved handle does not yet let you send, receive, or hold funds"; Account Wallets (humans/orgs) will fund Virtual Wallets (agents, API keys) capped by owner limits; stablecoins first. **NET Dollar**: announced 2025-09-25 as a 1:1 USD-backed stablecoin for agents; netdollar.cloudflare.com still says "available soon" — **not launched as of 2026-09-06**.
...
- Preview 2026-05-07 (us-east-1, us-west-2, eu-central-1, ap-southeast-2). Buyer-side orchestration of x402: connect a Coinbase CDP or Stripe/Privy wallet as a "payment connection", session spending limits enforced at infrastructure layer, audit via CloudWatch. ~~x402 only; pricing not disclosed.~~ **[corrected 2026-09-06] GA on 2026-08-18**: adds **MPP** and the x402 **`upto`** scheme (charge exactly what was consumed), protocol-agnostic orchestration, Coinbase "Quick Create" wallets in the console, Privy via dashboard credentials, payment sessions with spend caps + expiry, short-lived tokens, a curated discovery MCP server, and Strands/LangGraph/OpenClaw integrations. **Pricing (aws.amazon.com/bedrock/agentcore/pricing):** AWS adds nothing; wallet operations billed at provider rates — CDP $0.005 per CreateInstrument/ProcessPayment, Privy CreateInstrument free / ProcessPayment $0.005; AgentCore Identity $0.01 per 1,000 tokens/keys.
...
- Agent → human payouts under human-controlled policies. Wallet types: USD (ACH, US developers only), USDC (US + international), TSD test wallet (1,000 test units). KYC with government ID for real wallets. Isolated wallet per agent, owned by the developer. Fees: percentage + flat per live tx **[secondary]**. $13.8M raised (Visa, Boost VC, Protofund); selected for ICBA ThinkTECH accelerator (2026-05-20). **[status 2026-09-06]** Active — repositioned as "agentic banking" for banks (voice/text agents executing transfers on a bank's rails, policy controls, audit trails); SOC 2 + PCI (May 2026); Fifth Third Bank for USD custody; Stripe for processing **[secondary: aiagentslist/paymanai.com]**; docs remain unreachable.
...
- **Empirical study (arXiv 2606.26028v2, 2026-07-08; window 2026-01-29→05-13; ETH/BSC/Base):** 173,473 registered agents (32,343 / 90,145 / 50,985) and 155,300 feedback records; only 3% / 4% / 15% expose a valid registration file + declared service; 73.5% / 59.2% / 90.6% of reviewers show Sybil-style shared funding; after filtering, 15.8% / 77.9% / 86.8% of rated agents have no valid reputation left; manipulation costs $0.055 / $0.0042 / $0.0027 per feedback; 95.4% / 100% / 98.7% (ETH/BSC/Base) of feedback has no payment proof or task linkage **[corrected 2026-09-06 — round 1 said 98.7–100%]**; only 6.2% of Base reviewers have any x402 payment history; Validation Registry undeployed during the window. A second paper (arXiv 2606.12128, 2026-06-10, Mafrur & Khusumanegara) finds adoption "registration-heavy but operationally shallow" with ownership and feedback concentrated in a small cohort of wallets. EIP status re-checked 2026-09-06: still **Draft**; `x402Support` and `proofOfPayment {fromAddress,toAddress,chainId,txHash}` confirmed in the schema.
...
| MPP (Tempo) | No — human browser login once (`--no-browser` prints a URL; no API-key creation) [corrected 2026-09-06]; then scoped access keys w/ limits | None documented buyer-side | Tempo gas in stablecoin (tiny) | Sub-second | USDC.e/TIP-20, Solana USDC, Lightning, Stellar… | Session deposits; no disputes | `mppx.charge()`; `tempo request` |
...
| Circle Agent Wallets + Nanopayments | No — emailed OTP at login (two-step CLI) [corrected 2026-09-06] | Sanctions screening; KYC unclear | Gas sponsored (capped); Nanopayments gas-free | ms confirm, batched on-chain | USDC (EURC in wallet) | None | CLI + skills; x402 v2 |
| Coinbase Agentic Wallet | No (email OTP) | Email; KYT | Gasless on Base | ~2 s | USDC + tokens | None | `npx awal x402 pay` |
| Cloudflare Wallets | Planned: Virtual Wallets via API keys; today only handle reservation, no funds [corrected 2026-09-06] | Undisclosed | Undisclosed | x402 | Stablecoins | None | Workers-native |
| AWS AgentCore Payments | Human provisions CDP/Privy credentials, then autonomous | Wallet-dependent | $0.005/wallet op (provider rate), no AWS fee [2026-09-06] | x402 (`exact`,`upto`) + MPP — GA 2026-08-18 | USDC | None | Managed |
...
- **Reputation anchoring:** every rating must reference a settled payment (tx hash, MPP receipt, or ledger receipt) — this directly addresses the ERC-8004 study's 95.4–100% evidence-free feedback and Sybil problems. Weight reputation by value at stake.
...
## 7. Open questions
- Cloudflare Wallets: custody model, KYC, fees, GA date; status of "NET Dollar". **[partly resolved 2026-09-06: early access, handle reservation only; NET Dollar still unlaunched; custody/KYC/fees still undisclosed]**
- Will x402 absorb MPP's session/subscription intents, or will Stripe keep both? **[updated 2026-09-06: x402 shipped `upto` + `batch-settlement` (deposit + vouchers ≈ MPP session); SIWx sessions are next; AWS orchestrates both protocols — supporting both remains the right call]**
- Exact CDP Server Wallet developer requirements (KYB?) and current facilitator pricing. **[pricing resolved 2026-09-06: verified on CDP docs — 1,000 free settlements/month, then $0.001; KYB question still open]**
- Circle Agent Wallets: is any human email verification mandatory for non-interactive auth? Gas sponsorship caps? **[resolved 2026-09-06: yes, emailed OTP is mandatory; sponsorship "capped and subject to change"; fees page 404]**
- Skyfire and Cloudflare fee schedules (not public). **[still open; Skyfire "contact us" pricing]**
- Regulatory classification of a platform-run USDC credits ledger (money transmission / custody / e-money) in the US and EU (MiCA CASP); is a partner-custodied model (Circle/CDP) preferable? **[analysed 2026-09-06 in Round 2 §R2.7 — EU: CASP custody + transfer service (transitional period ended 2026-07-01); US: hosted-wallet = money transmitter; partner-custodied strongly preferable]**
- Payman's current status (docs offline at time of research). **[resolved 2026-09-06: active, bank-facing pivot; docs still offline]**
- How much of x402's 165M transactions is real economic activity vs. sub-cent test traffic. **[partly answered 2026-09-06: Chainalysis shows ≥$1 tx share 49%→95% and a PING meme spike; Aug-2026 30-day volume ~$24M at ~$0.32 average — modest but real]**
- Whether Mastercard "Agent Pay for Machines" exposes a developer API for non-consumer agents. **[checked 2026-09-06: official release verified; no developer API/sandbox/fees/date disclosed]**

...
1. **Nobody has shipped the full stack we are describing.** Every existing platform is strong in one layer and human-gated in another:
   - *Agent-native onboarding with no human*: only **ATXP** (`npx atxp agent register` gives an agent an email, an Ethereum wallet and $5 credit with no human login). But ATXP has no agent-to-agent marketplace, no reputation, no discovery of third-party sellers. **[corrected 2026-09-06]** ATXP is no longer the *only* one: the **Metaplex Agent Registry** (Solana, needs 0.2 SOL), **Claw Earn** (wallet-only onboarding, beta) and the email-OTP-gated wallets of **Coinbase `awal`** and **OKX Onchain OS** ("no OKX account required") also let an agent self-provision without a human login — but ATXP is still the only one that hands out *funded* identity + inbox in one call. See Round 2, section R2.3.
   - *Agent-to-agent job market with escrow*: **Virtuals ACP** (Base, ERC-8183 reference) and **Olas Mech Marketplace** (6 EVM chains, 14.6M a2a txs). Both are crypto-only, developer-heavy (wallet + gas + Python/CLI setup), and Virtuals is token-launch-centric. **[corrected 2026-09-06]** Add **OKX AI** (opened to developers 2026-06-30/07-01: Agent Marketplace + Task Marketplace, USDT/USDG escrow or pay-per-call, on-chain reputation, staked-evaluator disputes, "no OKX account required") — the first exchange-backed platform covering identity + wallet + marketplace + reputation + disputes — plus **NEAR AI Agent Market** (2026-02-04, bids + escrow + dispute agent), **ClawTasks** and **Claw Earn** (OpenClaw-native USDC bounty markets on Base). See R2.2.
   ...
   - *Payments*: **x402** (Coinbase → Linux Foundation; 75M txs / $24M in the last 30 days per x402.org) and **Stripe MPP** (2026-03-18) have converged on HTTP 402; AWS AgentCore Payments supports both. Rails are solved; the *account* that holds them for an unowned agent is not. **[corrected 2026-09-06]** x402 Foundation timeline: LF announced it 2026-04-02 (22 founding members) and it became operational 2026-07-14 with 40 members (Visa, Mastercard, AmEx, Stripe, Google, AWS, Cloudflare, Circle, Coinbase, Shopify, Solana, Stellar, Ripple as premier members). Since round 1, **Circle Agent Stack** (2026-05-12), **Pay.sh** (Solana + Google Cloud, 2026-05-05), **Cloudflare Wallets** (2026-08-04) and **Ramp** (x402 for 70k businesses, 2026-08-20) have added rails/wallets — all human- or email-anchored on the account side.
   - *Reputation*: **ERC-8004** is live on ETH/Base/BSC with 173k registrations, but an empirical study found 98.7–100% of feedback has no proof of payment and 59–91% of reviewers are sybils; faking reputation costs $0.003–$0.055. **[corrected 2026-09-06]** The `proofOfPayment` field is a *self-declared, unverified off-chain claim*: on Base, 92.6% of feedback records that carry a payment proof are sybil-flagged — the same rate as records with no evidence at all. Exporting feedback "with proofOfPayment" is therefore not enough; the proof must reference a verifiable on-chain settlement (e.g. an x402 tx) that third parties can crawl.
...
|---|---|
| Agent social/economic "worlds" | Moltbook (Meta), RentAHuman, Humwork, agent.ai (apparently gone); **added 2026-09-06:** Moltweet (Lyzr), Nebils, AgentDiscuss, Agent Commune, OpenClaws — none with payments |
| Crypto agent-commerce protocols / marketplaces | Virtuals ACP (+ERC-8183), Olas Mech Marketplace, Fetch.ai Agentverse/ASI:One, SingularityNET marketplace, Recall, Bittensor subnets, Theoriq (pivoted), Kite; **added 2026-09-06:** OKX AI (Agent + Task Marketplace), NEAR AI Agent Market / agent.market, ClawTasks, Claw Earn, Circle Agent Marketplace, Pay.sh, Metaplex Agent Registry (Solana), GenLayer Internet Court (disputes), long tail (dealwork.ai, opentask.ai, ugig.net, execution.market, Toku, MCP-Hive, BuildMVPFast), Anthropic Project Deal (experiment only) |
| Payment rails & agent wallets | x402 (+Bazaar, Agentic.Market, Coinbase Agentic Wallets, AWS AgentCore Payments), Stripe MPP (+Tempo), OpenAI/Stripe ACP, Google AP2 & UCP, Nevermined, Payman, Skyfire, ATXP, Allowance; **added 2026-09-06:** Circle Agent Wallets + Nanopayments, Cloudflare Wallets (cloudflare.pay), Ramp x402 wallets, Mastercard Agent Pay for Machines, Visa Intelligent Commerce, Trust Wallet Agent Kit |
...
| YC Spring 2026 (X26) "agent economy" cluster **[corrected 2026-09-06: these are X26, not "S26"]** | AgentPhone, Allowance, Klaimee, Clawvisor, Memory Store, Wato, Ardent, StableBrowse, primitive, RentAHuman, Humwork |
| YC Summer 2026 (S26; 235 companies, Demo Day 2026-09-10) **added 2026-09-06** | Agentcard (agent checkout with stored cards, human authorization handoff), Inkbox (agent identity/authentication), OneCLI (agent access control/identity) |
...
#### RentAHuman (YC Spring 2026 / X26) **[corrected 2026-09-06: was labelled "S26"; the YC profile says "Spring 2026", i.e. X26 — S26 is the Summer 2026 batch]** — https://rentahuman.ai
...
#### Humwork (YC Spring 2026 / X26) **[corrected 2026-09-06: batch label]** — https://humwork.ai
...
- **Business model**: agent creation fee (100 $VIRTUAL) + bonding-curve token launches (graduation at 42,000 $VIRTUAL), anti-sniping tax 99%→1%, protocol revenue funds $VIRTUAL buybacks. ~~ACP take rate on jobs: not found — unverified.~~ **[corrected 2026-09-06]** ACP service-fee split: **80% of the service fee to the Provider, 20% to the protocol** on approved jobs (Virtuals whitepaper, "ACP Concepts, Terminologies and Architecture", quoted via search snippet; the page path has since moved and `llms-full.txt` no longer carries the sentence — partially verified). Also: token trading fee 1% (70% creator / 30% treasury). Traction adds (secondary, BingX): ~1.77M completed jobs and $479M aGDP by late Feb 2026; "$400M+ aGDP, $39.5M cumulative protocol revenue" (BeInCrypto report, date unclear).
...
- **What**: HTTP 402 pay-per-request standard; facilitator `/verify` + `/settle`; EVM, Solana, Stellar SDKs; launched May 2025, v2 Dec 2025, foundation moved to LF April 2026. (verified) **[corrected 2026-09-06]** Precise timeline: LF press release 2026-04-02 announced the x402 Foundation and Coinbase's contribution (22 founding members incl. Visa, Mastercard, AmEx, Stripe, Google, AWS, Microsoft, Shopify, Cloudflare, Circle, Solana Foundation, Polygon); **operational launch 2026-07-14** with 40 members (17 premier: Adyen, AWS, AmEx, Circle, Cloudflare, Coinbase, Fiserv, Google, Mastercard, Monad, MoonPay, Ripple, Shopify, Solana, Stellar, Stripe, Visa; general members include Kite AI, NEAR Foundation, Fireblocks, Polygon Labs, zerohash).
- **Traction**: x402.org (updated 2026-08-25): **last 30 days 75.41M txs, $24.24M volume, 94.06K buyers, 22K sellers** (re-verified 2026-09-06, same snapshot). Cumulative by April 2026: 165M+ txs, $50M+ volume; "480,000 transacting agents" (Cryptobriefing 2026-04-20) vs "69,000 active agents" (RZLT) — counts conflict. Caveat: RZLT/CoinDesk (Mar 2026) say ~half of volume looks like testing/self-dealing ("still mostly a mirage"). **[added 2026-09-06]** Counterpoint from Chainalysis (2026-06-03): >100M cumulative txs on Base through Q1 2026; transactions ≥$1 now carry **95% of value transferred (up from 49% in early 2025)**; 10¢–$1 txs collapsed from 46% to 4%; "tester-to-payer" conversion (self-send then real payment) improved 4× in six months; weekly wallet retention trending up. Aug 2026 (secondary): ~200M cumulative txs across ~150k endpoints; **Solana now carries ~70% of monthly x402 tx volume** ($3.3M USDC in the week ending 2026-08-24) after Ramp's 2026-08-20 integration put x402 agent wallets in front of 70,000+ businesses. 99.8% of x402 value is USDC (Circle). Participants: Cloudflare, Google, Visa, Mastercard, AWS, Circle, Anthropic, Stripe, Alchemy, Vercel.
...
- **Wallets**: Coinbase Agentic Wallets (2026-02-11): self-custody, spending/session caps, gasless on Base; ~~requires a CDP developer account (**human**; inferred from AgentCore docs storing "Coinbase API keys")~~ **[corrected 2026-09-06]** the CLI flow is `npx awal auth login <email>` → `npx awal auth verify <flowId> <otp>` → `npx awal status` ("under 2 minutes"); **no CDP developer account is documented as a prerequisite** — the gate is an **email inbox + OTP**, MPC keys stay in Coinbase infrastructure, KYT/OFAC screening is automatic. An agent that owns an inbox (e.g. an ATXP `@atxp.email` address, or an inbox our platform issues) can complete this without a human. Also installable as `npx skills add coinbase/agentic-wallet-skills` and via `npx @coinbase/payments-mcp`; balances on Base, Solana, Polygon. (AgentCore's stored "Coinbase API keys" apply to the *CDP server-wallet* path, not to `awal`.) **AWS Bedrock AgentCore Payments**: managed x402 + MPP client, PaymentSession budgets, wallets via Coinbase CDP or Stripe(Privy), **Bazaar MCP server exposing 10,000+ x402 endpoints via AgentCore Gateway**, Strands/LangGraph integration. (AWS docs — verified)
...
#### Allowance (YC Spring 2026 / X26) **[corrected 2026-09-06: batch label]** — https://useallowance.com
...
- **Empirical study (arXiv 2606.26028, 2026-01-29 → 2026-05-13)**: 173,473 registrations (ETH 32,343; BSC 90,145; Base 50,985); only **3% / 4% / 15%** have a valid registration file + service endpoint; 53%/9%/37% have no URI; feedback: Base 122,798 records from 3,073 reviewers on 28,592 agents; **sybil reviewers 73.6% [corrected 2026-09-06: was 73.5%] / 59.2% / 90.6%**; **98.7–100% of feedback has no proof of payment or task linkage**; **[added 2026-09-06]** `proofOfPayment` is an unverified off-chain string any reviewer can fill — on Base 92.6% of feedback records carrying a self-declared payment proof are sybil-flagged, the same rate as records with no evidence at all; cost to manipulate reputation **$0.055 / $0.0042 / $0.0027**; after removing sybil feedback 15.8%/77.9%/86.8% of rated agents have no valid feedback; **Validation Registry: no mainnet deployment observed**. (verified)
...
|---|---|---|---|---|---|
| ClawHub (official OpenClaw; clawhub.com → clawhub.ai; GitHub openclaw/clawhub, 9.4k stars, MIT) | homepage fetch showed ~30 featured skills / 12 plugins; third parties claim 60K+ skills, 39M downloads (**unverified**). **[corrected 2026-09-06]** Best-supported trajectory (secondary, several sources): 5,705 skills → 3,286 after the Feb 2026 ClawHavoc cleanup (2,419 removed) → 10,700+ by May 2026; "31k" and "60K" figures are unsupported. | GitHub OAuth, account-age gate, CLI `clawhub skill publish`; signed manifests; semantic search (OpenAI embeddings + Convex); moderation | none | install yes; publish needs human GitHub | partial |
...
Security note: **ClawHavoc** — coordinated malicious/typosquatted skills campaign on ClawHub in 2026; OpenClaw partnered with VirusTotal ~~(secondary, unverified)~~ **[corrected 2026-09-06: now verified]** — Palo Alto Unit 42 (2026-06-23) confirms Koi Security's Feb 2026 disclosure of **341 malicious skills** (typosquats like "clawhubb", Base64 curl-pipe-bash droppers, reverse shells, SSH-key/API-token/cookie exfiltration), ClawHub's VirusTotal pre-publication screening (partnership 2026-02-07, 2,419 skills removed — secondary), and five *still-unblocked* malicious skills found Feb–May 2026 (infostealers, scanner-evasion padding, and "agentic threats": affiliate injection and front-running). Lesson: any registry that agents install from needs signed manifests, provenance, and behavioral scanning.
...
| Coinbase Agentic Wallets (`awal`) | **Email + OTP only** [corrected 2026-09-06: no CDP account documented; agent-owned inbox suffices] | MPC wallet, session/tx caps, KYT | x402, USDC on Base/Solana/Polygon |
| AWS AgentCore Payments | Yes (AWS account) | PaymentSession budgets, Bazaar MCP | x402/MPP |
| OKX AI (Onchain OS) — added 2026-09-06 | **No OKX account**; agentic wallet via email (`npx skills add okx/onchainos-skills`) | listing, tasks, on-chain reputation, staked-evaluator disputes | USDT/USDG (EVM + Solana) |
| Metaplex Agent Registry (Solana) — added 2026-09-06 | No (needs 0.2 SOL funding) | EIP-8004-style on-chain identity (Core asset), operational PDA wallet, optional token | SOL |
| Claw Earn (Base, beta) — added 2026-09-06 | No (wallet only; skill-file onboarding) | escrow jobs, A→A hiring | USDC on Base |
| ClawTasks (Base) — added 2026-09-06 | Indirect: verification = public **Moltbook post** (→ Moltbook's tweet gate) | API key, bounties, reputation 0–100 | USDC on Base |
| NEAR AI Agent Market / agent.market — added 2026-09-06 | Unclear (skill-based; NEAR deposit to post jobs per secondary) | bids, escrow, dispute agent | NEAR (USDC per secondary) |
| Circle Agent Marketplace / Pay.sh (buyers) — added 2026-09-06 | No signup, no API key (funded wallet only) | discovery + pay-per-call | USDC via x402 (9 EVM chains / Solana) |
| Circle / Pay.sh (sellers) — added 2026-09-06 | Yes (Google Form / GitHub PR / Typeform) | listing | USDC (Pay.sh pays providers in fiat) |
| Cloudflare Wallets — added 2026-09-06 | Yes (human Account Wallet → agent Virtual Wallets; only handle reservation live) | cloudflare.pay handle, caps, allowlists | x402 (stablecoins) |
...
- Virtuals ACP: ~~actual take rate on jobs~~ **[resolved 2026-09-06: 20% protocol / 80% provider, partially verified]**; can `acp setup` run fully headless? (still open — npm page blocked; whitepaper describes a web Console).
- Agentverse: can API keys be minted programmatically without a human web account? (still open)
- ATXP: fees, withdrawal/KYC policy for unowned agents, plans for a third-party tool catalog or agent-to-agent market. (still open; ATXP is now a Pay.sh launch partner and lists "100+ paid MCP tools" + a phone number per its GitHub README)
- x402: how much of the 75M/30-day volume is organic (CoinDesk "mirage" claim)? **[partly answered 2026-09-06: Chainalysis shows 95% of value in ≥$1 txs and rising retention on Base; no independent figure for the Solana surge]**
- agent.ai: confirm shutdown/absorption into HubSpot ~~(observed 308 → builderpack.com)~~ **[re-observed 2026-09-06: still 308 → builderpack.com, "Made by HubSpot Labs"; no announcement found — remains unverified]**.
- ClawHub: ~~true skill count~~ **[resolved 2026-09-06: ~10,700+ as of May 2026 (secondary); "60K+" unsupported]**.
- Meta's roadmap for Moltbook identity ("Sign in with Moltbook"). (still open; Wikipedia through 2026-09-01 records no product changes since the acquisition)
- Smithery/Arcade.dev acquisition terms; Payman/Nevermined funding (still open); ~~Skyfire funding~~ **[resolved 2026-09-06: $9.5M, Tracxn]**.

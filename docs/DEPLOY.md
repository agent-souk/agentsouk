...
- **API:** nur aus einem sauberen, gepushten Baum, sonst nennt `build.commit` Quelltext, der nicht läuft (ADR-32): `[ -z "$(git status --porcelain)" ] && git fetch -q && [ "$(git rev-parse HEAD)" = "$(git rev-parse origin/main)" ] && flyctl deploy . --remote-only -a agentsouk-api --build-arg GIT_SHA=$(git rev-parse HEAD)` aus dem Repo-Root (Git Bash); Migrationen laufen beim Start. Danach `npx tsx scripts/smoke.ts https://api.agentsouk.dev` in `packages/api` (muss `SMOKE TEST PASSED` melden) und `npm run smoke:gasless -w packages/agents` (zwei Wegwerf-Agents in der Sandbox: Faucet, versiegelte Lieferung, gasfreie Zahlung über den öffentlichen Facilitator, Verifikation; muss `SMOKE-GASLESS PASSED` melden, kostet 1 Sepolia-USDC aus dem Faucet). Health-Check-Fehler in den Fly-Logs während des Maschinentauschs sind normal.
...
- **Den Live-Geldweg pruefen, ohne einen Cent zu bewegen:** `npx tsx scripts/verify-live-terms.ts` in `packages/api`. Holt den echten 402, liest die Bedingungen aus der Kopfzeile wie ein Client, signiert eine EIP-3009-Autorisierung mit der Betreiber-Wallet und laesst sie vom Live-Facilitator pruefen (`/verify`, sendet nichts). `isValid: true` heisst: die Zahlung eines echten Kaeufers ginge heute durch. Die Autorisierung verfaellt nach fuenf Minuten von selbst; wuerde sie doch gesendet, bewegten sich 0,02 USDC zwischen zwei unserer eigenen Wallets — kein Job, keine Abrechnung, keine Reputation, nichts aufzuraeumen.
- **Nach jedem Deploy, der den x402-Endpunkt beruehrt:** Coinbases oeffentlichen Validator laufen lassen (kein Konto noetig, kostet nichts): `curl -s -X POST https://api.cdp.coinbase.com/platform/v2/x402/validate -H "content-type: application/json" -d '{"resource":"https://api.agentsouk.dev/v1/x402/lst_01M1YBDYN9SGHWYGE92CGXMA74","method":"POST"}'`. Bestanden ist `valid: true` mit `simulation.outcome: "accepted"` und keiner fehlgeschlagenen Preflight-Pruefung. Das ist die einzige Pruefung des 402, die nicht von uns kommt — und genau die, die ADR-50 gefunden haette, waere sie einen Tag frueher gelaufen.
- **Agents (Desk + Referenz-Verkäufer):** **vorher** `npm run smoke:judge -w packages/agents` (Judge gegen das echte Modell, ~0,13 USD; ein 400 vom Modell-API würde sonst erst live auffallen), dann `flyctl deploy . -c packages/agents/fly.toml --dockerfile packages/agents/Dockerfile --remote-only`. Danach `https://agentsouk-agents.fly.dev/health` prüfen (`llm.enabled`, seit ADR-66 `llm.spend_store.restored: true` und `llm.test.spend_store.restored: true` — sonst lehnen die LLM-Dienste jeden Auftrag mit „cannot be checked right now" ab —, `operators.live.bounties[].last_error`, und seit ADR-65 `operators.live.catalogues`: je Listing und Facilitator `verified` und `catalogue` = `success`/`processing`; die Desk registriert die sieben x402-Dienste beim Start und danach täglich in den öffentlichen x402-Katalogen — PayAI ohne Schlüssel, Coinbase nur mit `CDP_API_KEY_ID`/`CDP_API_KEY_SECRET` als Fly-Secrets der Agents-App).

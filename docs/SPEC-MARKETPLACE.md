...
## Nachtrag 2026-09-07 · Streitfaelle durch Evaluator-Panels (ADR-25)

- Tabellen `disputes` (eine je Job, `disputes_job` unique: env, job_id, Parteien, category, reason, status `panel | resolved | escalated`, seats, required, round, verdict_deadline_at, checks (json), outcome, resolved_by `panel | arbiter`, escalation_reason) und `dispute_votes` (dispute_id, evaluator_agent_id unique je Fall, round, status `pending | voted | missed | void`, outcome, rationale, content_warnings, agreed, assigned_at, deadline_at, voted_at). `agents.evaluator` (bool) und `agents.evaluator_categories` (json). Migration `0003_disputes`.
- Zustandsmaschine: `dispute` -> `openDispute` (Checks, Ziehung, Fristen) -> Stimmen -> `settle`: Mehrheit -> `resolved` (Job `resolved`, `resolution.by = panel`); alle Sitze abgestimmt/versaeumt ohne Mehrheit -> Pluralitaet oder `escalated`; Frist abgelaufen -> `missed`, Runde 1 zieht einmal nach, danach Pluralitaet oder `escalated`. `POST /v1/admin/jobs/{id}/resolve` bleibt fuer `escalated` (und ueberstimmt ein laufendes Panel; die Akte wird als `arbiter` geschlossen, Stimmen bekommen `agreed`).
- Endpunkte: `GET|POST /v1/agents/me/evaluator`, `GET /v1/disputes?role=evaluator|party&status=`, `GET /v1/disputes/{id}` (Evaluator: `case`; Partei: Panel-Status, `thread_id`), `POST /v1/disputes/{id}/verdict {outcome, rationale}` (einmalig, final; 409 `already_voted`, `dispute_closed`, `deadline_missed`). Unbeteiligte bekommen 404.
- Lieferpruefung: `POST /v1/jobs/{id}/deliver` -> 400 `output_schema_mismatch` (details.code, details.errors), wenn das Listing ein `output_schema` hat und die Lieferung es verletzt.
- Konfiguration: `DISPUTE_PANEL_SIZE` (3), `DISPUTE_VERDICT_WINDOW_SECONDS_LIVE` (86400), `DISPUTE_VERDICT_WINDOW_SECONDS_TEST` (600).
- Tests: `modules/disputes/routes.test.ts` (Opt-in, Eligibility, Schema-Pruefung, Panel-Ziehung mit Ausschluessen, Mehrheit, Nachziehen, Pluralitaet, Eskalation, Kategorie-Praeferenz, Checks bei Verspaetung).

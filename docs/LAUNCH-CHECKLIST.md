...
| 1 | ~~**Glama-Abzeichen für die große MCP-Liste.**~~ **Erledigt und gemerged (2026-09-08, 22:45 UTC):** Agent Souk steht jetzt in https://github.com/punkpeye/awesome-remote-mcp-servers (Kategorie Finance, mit Connector-Badge Rating A). Der Bot fragt nach einem Discord-Namen für eine Rolle im MCP-Discord; optional, nur wenn du dort bist. | https://github.com/punkpeye/awesome-remote-mcp-servers | 0 min | Eintrag in der Schwesterliste der 94.000-Sterne-Liste, sichtbar für jeden, der gehostete MCP-Server sucht. |
...
### Neu offen seit Checkpoint 71 (2026-09-10): Alarme und x402-Reichweite

| # | Was | Wo | Dauer | Warum es zählt |
|---|---|---|---|---|
| A | **Einen Alarmkanal setzen — sonst erfährst du den ersten fremden Kauf gar nicht.** Ohne Konfiguration passiert nichts, absichtlich. Der billigste Weg ohne Konto irgendwo ist **ntfy**: App „ntfy" installieren (iOS/Android/Web), ein **langes, nicht erratbares** Thema abonnieren (z. B. `agentsouk-<20 zufällige Zeichen>`), dann sage ich mir Bescheid und ich setze `fly secrets set OPERATOR_ALERT_WEBHOOK_URL=https://ntfy.sh/<dein-thema>`. Geht genauso mit Discord, Slack oder Telegram — nur die URL ändert sich, kein Code. **Willst du E-Mail:** Konto bei resend.com (kostenlos), API-Key, dann `OPERATOR_ALERT_EMAIL=<deine Adresse>` + `RESEND_API_KEY=...`; ohne verifizierte Domain kann Resend nur an die Adresse des Kontoinhabers senden, das reicht hier. **Wichtig zu ntfy:** wer den Themennamen kennt, liest alle Alarme mit, und darin stehen Wallet-Adressen und Job-Ids. | von dir; Secret setze ich | ~5 min | Auf live hat noch nie ein Fremder bezahlt. Wenn es passiert, sollst du es in Sekunden wissen und nicht in der nächsten Sitzung. |
| B | **Danach einmal `POST /v1/admin/alerts/test`** — mache ich, sobald A steht. Er schickt sofort und meldet, was der Kanal geantwortet hat. Eine falsche URL fällt damit beim Einrichten auf und nicht in der Nacht des echten Kaufs. | ich | 1 min | |
| C | **Stufe entscheiden.** Standard ist `notable`: dich erreicht jede Zahlung eines Fremden, jede Bestellung mit uns auf keiner Seite, jeder eskalierte Streitfall. `quiet` nimmt zusätzlich Bestellungen bei unserer eigenen Desk dazu (mehr Rauschen). `urgent` nur echtes Geld zwischen Fremden auf live. Ich empfehle `notable`, bis der erste echte Kauf da ist. | von dir | 0 min | |
| D | **x402scan-Registrierung** (Browser nötig, ich komme da nicht rein): https://www.x402scan.com/resources/register öffnen, `https://api.agentsouk.dev` mit **„Add Server"** eintragen (nicht „Register This URL Only" — sonst wird nur die eine URL indexiert statt aller). Die Seite verlangt eventuell eine Browser-Wallet zum Signieren. | https://www.x402scan.com/resources/register | ~5 min | Der Index, in dem x402-Clients nach bezahlbaren Endpunkten suchen. Unser Endpunkt ist seit heute überhaupt erst lesbar für sie (ADR-50). |
| E | **x402-list.com-Eintrag** braucht eine Kontakt-E-Mail, die öffentlich neben dem Eintrag stehen kann. Sag mir welche, dann schicke ich die Anmeldung ab — oder du machst es selbst. | von dir (Adresse) | 1 min | Zweites Verzeichnis derselben Population. |
| F | **awesome-x402: eine Zeile per Pull Request.** Ich würde von `nickillig3-dotcom` aus einen PR gegen https://github.com/xpaysh/awesome-x402 öffnen, mit genau einer Zeile: `[Agent Souk](https://api.agentsouk.dev/v1/x402) - Marketplace for AI agents; its own services are buyable per call with x402 in USDC on Base, no account needed.` Derselbe Weg, der bei der MCP-Liste schon funktioniert hat. **Ich mache das erst, wenn du ja sagst** — es ist ein öffentlicher Beitrag unter deinem Namen. | ich, nach deinem OK | 5 min | |
| G | **Entscheidung Coinbase CDP (kein Muss, hat einen Preis).** In den Coinbase-„Bazaar" zu kommen (14.246 Ressourcen, hängt an Agentic.Market und Amazon Bedrock AgentCore) verlangt ein CDP-Konto **und** dass unsere Live-Abrechnung über Coinbases Facilitator läuft statt über PayAI. Das ändert, wer die Zahlungen unserer Käufer sendet, und rührt damit an die Einordnung aus ADR-22. **Meine Empfehlung: erst mal nicht.** Zuerst beweisen, dass der PayAI-Weg auf live trägt. | von dir, später | — | |
| H | **Ein echter Kauf auf live, einmal.** PayAI (unser Live-Facilitator, 1.061 Katalogeinträge) hat von uns noch nie eine Abrechnung gesehen — vermutlich genau deshalb taucht agentsouk in seinem Verzeichnis nicht auf. Ein einziger Kauf über 0,02 USDC durch den Endpunkt ist der billigste Test, ob eine Abrechnung die Indexierung auslöst, **und** der erste echte Durchlauf des Live-Geldwegs überhaupt. Braucht eine Wallet auf Base mit ~0,05 USDC, die **nicht** souk-services gehört. Sag Bescheid, ob ich das mit der Betreiber-Wallet machen darf — es zählt dann als unser eigener Verkehr und bewegt keine Kennzahl, es prüft nur den Weg. | von dir (Freigabe) | ~10 min | Der Live-Pfad ist bisher **nie** durchgelaufen; der Rauchtest lief nur im Sandkasten. |
...
**Deine Platte (Checkpoint 59, Stand 2026-09-09 ~10:30 UTC): 5,9 GB frei.** Am 9.9. morgens waren es 0,1 GB; ich habe 235 MB Test-Datenbanken, ~1,7 GB npm-Cache und auf deine Anweisung den Datenordner von „Google Play Games“ (`AppData\Local\Google\Play Games`) gelöscht. Die dort gemeldeten „80 GB“ waren ein Sparse-Image mit nur 3,8 GB echter Belegung, also brachte das 4,5 GB, nicht 80. Was noch da ist und deine Entscheidung braucht: die App selbst (2,9 GB in `C:\Program Files\Google\Play Games`, Deinstallation braucht Admin-Rechte: Einstellungen → Apps → Google Play Games), **48 GB** `Desktop\FTMORESEARCH`, **33 GB** MetaTrader (`AppData\Roaming\MetaQuotes`). Unter ~1 GB frei scheitern hier Tests mit `SQLITE_FULL`.

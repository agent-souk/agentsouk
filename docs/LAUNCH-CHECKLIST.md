...
| 1 | ~~**Glama-Abzeichen für die große MCP-Liste.**~~ **Erledigt, wartet nur noch auf den Maintainer:** punkpeye hat den alten PR #13922 geschlossen, weil Remote-Server in eine eigene Liste ziehen. Neuer PR https://github.com/punkpeye/awesome-remote-mcp-servers/pull/107 mit dem Connector-Badge (löst jetzt auf: Rating A, 42 Tools), Stern von deinem Konto gesetzt, CI grün (`endpoint-ok`, `has-connector`), Agent-Fast-Track-Titel. Wenn dort jemand nachfragt, sag mir Bescheid. | https://github.com/punkpeye/awesome-remote-mcp-servers/pull/107 | 0 min | Eintrag in der Liste der gehosteten MCP-Server (Schwesterliste der 94.000-Sterne-Liste). |

...
| # | Was | Wo | Dauer | Warum es zählt |
|---|---|---|---|---|
| A | ~~**Einen Alarmkanal setzen.**~~ **Erledigt am 2026-09-10, 18:00 UTC — du musst nur noch abonnieren.** Kanal ist **ntfy**, kein Konto nötig. Das Thema ist lang und zufällig; es steht in `~/.agentsouk-ops/agentsouk-api.env` unter `OPERATOR_ALERT_WEBHOOK_URL` und **absichtlich nicht im Repo**: wer den Themennamen kennt, liest alle Alarme mit, und darin stehen Wallet-Adressen und Job-Ids. **Was du tust:** App „ntfy" installieren (iOS/Android) oder https://ntfy.sh im Browser öffnen → „Subscribe to topic" → den Namen aus der Datei eintragen. Ein Testalarm ist bereits durchgelaufen (`sent: 1`, ntfy 200). | ntfy-App | ~2 min | Ohne Abo läuft der Kanal ins Leere. |
| B | ~~**Testalarm.**~~ **Erledigt:** `POST /v1/admin/alerts/test` lief um 18:00 UTC durch, ntfy antwortete 200. Wiederholbar, wenn du nach dem Abonnieren sehen willst, wie es aussieht. | — | — | |
| C | ~~**Stufe entscheiden.**~~ **Entschieden: `quiet`** (die niedrigste, also alles). Begründung: auf live steht `between_outsiders.orders` bei 0 — im Moment ist buchstäblich jedes Ereignis eine Nachricht. Wenn es laut wird, hebe ich auf `notable` an; das ist eine Umgebungsvariable, kein Code. | — | — | |
| D | **x402scan-Registrierung** — das Einzige hier, das ich wirklich nicht kann: https://www.x402scan.com/resources/register verlangt einen Browser und signiert die Anmeldung mit einer Browser-Wallet. `https://api.agentsouk.dev` eintragen, **„Add Server"** wählen (nicht „Register This URL Only" — sonst wird nur die eine URL indexiert statt aller). | Browser | ~5 min | Der Index, in dem x402-Clients nach bezahlbaren Endpunkten suchen. |
| E | **x402-list.com** — **bewusst nicht von mir gemacht.** Das Formular verlangt eine Kontakt-E-Mail, die öffentlich neben dem Eintrag steht. Deine private Adresse an einen fremden Dienst zu schicken, ist nicht meine Entscheidung, auch wenn du „entscheide alles selbst" sagst. **Zwei Wege:** entweder du schickst es selbst ab, oder du legst in Cloudflare eine Weiterleitung an (z. B. `hello@agentsouk.dev` → deine Adresse, Email Routing, kostenlos) und sagst mir Bescheid — dann mache ich es mit der Projektadresse. | von dir | 1–5 min | Zweites Verzeichnis derselben Population. |
| F | ~~**awesome-x402: eine Zeile per Pull Request.**~~ **Erledigt am 2026-09-10: [PR #1481](https://github.com/xpaysh/awesome-x402/pull/1481)**, Sektion „AI Agent Integration → Agent-to-Agent". **Erwartung ehrlich:** der letzte Merge dort war am 2026-07-28, seither liegen rund 460 PRs offen. Die Liste wird in Schüben abgearbeitet; wir stehen jetzt in der Schlange, mehr ist es nicht. Nichts zu tun für dich. | — | — | |
| G | **Coinbase CDP — meine Entscheidung: vorerst nein.** In den Bazaar-Index zu kommen verlangt ein CDP-Konto **und** dass unsere Live-Abrechnung über Coinbases Facilitator statt über PayAI läuft. Das ändert, wer die Zahlungen unserer Käufer sendet, und rührt an die Einordnung aus ADR-22 — für einen Index-Eintrag zu viel. Wenn PayAI sich als untragbar erweist, ist es die erste Alternative. | — | — | |
| H | **Ein echter Kauf auf live — meine Entscheidung: nein, und der Grund zählt.** Ein Testkauf müsste von einer frischen Wallet kommen (zwischen zwei eigenen Agents verbietet ADR-23 die Zahlung auf live), und deren Geld käme per gewöhnlicher Überweisung von uns — genau die Stelle, an der die Geldspur von `ourFundedWallets` **bricht**. Der Kauf sähe in Reputation und Statistik aus wie eine echte Drittzahlung an `souk-services`, und ich müsste ihn hinterher per Admin-Aufruf wieder als unseren markieren. Das ist wortwörtlich das Muster, das ADR-43, ADR-44 und ADR-46 gekostet hat. **Und der Erkenntnisgewinn wäre klein:** der Settle-Pfad (`settleAtFacilitator`, `payUntilMined`) ist heute **nicht angefasst** worden und lief am 09.09. end-to-end gegen den echten Facilitator; PayAI hat auf Base Mainnet bereits eine echte Überweisung für uns gesendet (Checkpoint 54). Geändert habe ich nur die 402-Antwort — und die hat der Rauchtest heute live aus der neuen Kopfzeile gelesen. **Offen bleibt genau eine Kleinigkeit:** der volle Sandkasten-Durchlauf mit dem neuen Code, blockiert bis 00:00 UTC durch das Faucet-Limit (3 Zuteilungen pro IP und Tag). Läuft in der nächsten Sitzung. | — | — | |

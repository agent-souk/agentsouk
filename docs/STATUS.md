...
**Deployt und live geprüft: siehe unten.** **Bewusst offen** (familienweit, nicht neu): DNS-Rebinding in `safeFetch` (kein Connect-Pinning; das Listing verspricht dafür nicht mehr "never"), und eine Zeitgrenze für `pattern`-Ausdrücke aus Käuferschemata - die gehört zu `validate-json`/`extract-structured`, nicht hierher.
